 ...
 * @Description: 压缩文件操作
 * @Author: yy
 * @Date: Created in 2020/1/8 15:47
 * @Modified By：
 */
    ...
    public static void zipFiles(String srcDirName, String fileName, String descFileName) {
        ...
        try {
            ...
            if ("*".equals(fileName) || "".equals(fileName)) {
                ZipFileUtils.zipDirectoryToZipFile(dirPath, fileDir, zouts);
            } else {
                File file = new File(fileDir, fileName);
                if (file.isFile()) {
                    ZipFileUtils.zipFilesToZipFile(dirPath, file, zouts);
                } else {
                    ZipFileUtils.zipDirectoryToZipFile(dirPath, file, zouts);
                }
            }
            zouts.close();
    ...
    public static void zipDirectoryToZipFile(String dirPath, File fileDir, ZipOutputStream zouts) {
        if (fileDir.isDirectory()) {
            ...
            for (int i = 0; i < files.length; i++) {
                if (files[i].isFile()) {
                    // 如果是文件，则调用文件压缩方法
                    ZipFileUtils.zipFilesToZipFile(dirPath, files[i], zouts);
                } else {
                    // 如果是目录，则递归调用
                    ZipFileUtils.zipDirectoryToZipFile(dirPath, files[i], zouts);
                }
            }
        }
    }
